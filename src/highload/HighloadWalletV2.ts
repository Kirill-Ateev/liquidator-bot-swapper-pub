...
export function createHighloadWalletTransfer(args: {
    ...
    // Body
    const body = beginCell().storeBuffer(signature).storeBuilder(signingMessage).endCell()
...
export function createHighloadWalletTransferV2(args: {
    ...
    // Create message
    const signingMessage = beginCell().storeUint(args.walletId, 32).storeUint(queryId, 64)
...
export class HighloadWalletContractV2 implements Contract {
     ...
     * Create transfer
     */
    createTransfer(args: { secretKey: Buffer; messages: MessageRelaxed[]; sendMode?: Maybe<SendMode>; timeout?: Maybe<number> }) {
