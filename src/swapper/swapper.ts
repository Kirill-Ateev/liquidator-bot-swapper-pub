...
import {
  FetchSwapMessagesResult,
  HighloadExchangeResult,
  ParsedSwapResult,
  SwapMessage,
  SwapParams,
} from './types';
...
export async function fetchSwapMessages(
  ...
  if (maxSlippage <= 0 || maxSlippage >= 1) {
    throw new Error(
      `Slippage value must be in (0, 1), provided value ${maxSlippage} is not valid`
    );
  }

  if (maxLength < 2 || maxLength > 5) {
    throw new Error(
      `Max length value must be in [2, 5], provided value ${maxLength} is not valid`
    );
  }
  ...
  checkDefined(
    targetTransactions,
    'No messages fetched, probably server error'
  );
...
export async function checkSwapResult(routeId: number): Promise<number> {
  ...
  return data.reduce(
    (prev: number, curr: ApiTransactionResult): number =>
      prev | transformStatus(curr.status),
    0
  );
}
