...
import { Address, beginCell, internal, OpenedContract, storeStateInit, toNano, TonClient, WalletContractV4 } from '@ton/ton'
...
async function deployHighload(client: TonClient, wallet: OpenedContract<WalletContractV4>, subwallet_id: number, keys: KeyPair) {
    const dataCell = beginCell().storeUint(subwallet_id, 32).storeUint(0, 64).storeBuffer(keys.publicKey).storeBit(0).endCell()
    ...
    console.log(
        `Highload contract deployed: ${highloadAddress.toString({
...
;(async () => {
    ...
    console.log(
        'Wallet address:',
        walletContract.address.toString({
            testOnly: process.env.IS_TESTNET === 'true',
        })
    )

    deployHighload(tonClient, walletContract, config.subwalletId, keypair)
        .catch((e) => {
            console.log(e)
        })
...
// Deployed HighLoad wallet v2
// testnet: kQDY3g19Kvfdy1bdYXuCdJVzkeTxH_8A4v07kmIWmaM1nFeZ
// mainnet: UQDY3g19Kvfdy1bdYXuCdJVzkeTxH_8A4v07kmIWmaM1nLHW
